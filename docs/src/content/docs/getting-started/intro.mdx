...
Kod gives you a private Git server with CI-like workflows, token-based access control, collaborator SSH keys, webhook delivery tracking, and encrypted secrets -- all in a single portable service. No database servers, no container runtimes, no external services required.

## What's Included

- **Git Hosting**: HTTP Git with token authentication and SSH Git with collaborator public keys
...
- **Webhook Delivery**: Push/workflow webhooks with signatures, history, and retries
...
See the [Installation & Quickstart](/docs/getting-started/installation-quickstart) guide for the full walkthrough.
