...
export function createCollaboratorRoutes(db: Database): Route[] {
  return [listCollaborators(db), addCollaborator(db), removeCollaborator(db)];
}

// GET /repos/:name/collaborators - List collaborators
const listCollaborators = (db: Database): Route => ({
  method: 'GET',
...
const addCollaborator = (db: Database): Route => ({
  method: 'POST',
  pattern: /^\/repos\/(?<name>[^/]+)\/collaborators\/?$/,
  handler: async (
    req: HttpRequest,
...
const removeCollaborator = (db: Database): Route => ({
  method: 'DELETE',
